...
function _getPostings(options, markup) {
	...
	$('div.content').find('p.row').each(function (i, element) {
		var detailsUrl = $(element).find('span.pl a').attr('href');
		...
		posting = {
			...
			date: ($(element).find('span.pl time').attr('datetime') || '').trim(),
			hasPic: ($(element).find('span.l2 span.p').text() || '').trim() !== '',
			location: ($(element).find('span.pnr small').text() || '').replace(/[\(,\)]/g, '') // santize
			.trim(),
			pid: ($(element).attr('data-pid') || '').trim(),
			price: ($(element).find('span.l2 span.price').text() || '').replace(/^\&\#x0024\;/g, '').trim(), // sanitize
			title: ($(element).find('span.pl a').text() || '').trim(),
			url: detailsUrl
		};
